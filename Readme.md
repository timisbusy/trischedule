...
$$\forall \quad 0 \leq d \leq 6 \quad with \quad d_0 = max(0,d-5) \quad IncomingFatigue * max(0,(1 - (d * .25))) \quad + \quad  \sum_{w{_s}=0}^{m{_s}}  $$ $$^{m{_s}} \sum_{d{_0}}^d FatigueIncrease[w{_s}] * SelectedWorkouts[d, w{_s}]  \leq FatigueUpperBound$$
